...
- **macOS crash framing:** the current stress-crash risk is a residual reverse-P/Invoke suspension window, not `dotnet/runtime#44498` (fixed upstream around .NET 6) and not extra mruby 4.0 teardown work. mruby 4.0 shifted structural GC timing and crash probability; the mitigation is the per-state GCHandle registry, native `mrb_data_disarm`, callback-bridge allocation reduction, and `NoGCRegion` in the storm amplifier.
